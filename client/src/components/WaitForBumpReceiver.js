...
class WaitForBumpReceiver extends React.Component {
  ...
  handleAccelerationEvent = (event) => {
    ...
    if (
      isAccListenerActive &&
      (Math.abs(event.acceleration.x) > 9 || Math.abs(event.acceleration.y) > 8))
    {
    ...
    return (
      <div className="App">
        <button
          ...
          onClick={this.bamEvent}
          onKeyPress={this.spaceBamEvent}
        >
          <img
            src={FistsBumping}
            className="fists-bumping-image-size"
            alt="Fist Bump Waiting Pic"
          />
        </button>
